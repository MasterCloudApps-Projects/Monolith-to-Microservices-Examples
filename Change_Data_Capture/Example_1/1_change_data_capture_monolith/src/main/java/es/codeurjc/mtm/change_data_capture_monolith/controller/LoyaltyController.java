...
public class LoyaltyController {
  ...
  public ResponseEntity<Loyalty> createLoyalty(@RequestBody Loyalty loyalty) {
    Loyalty loyaltyStored = this.loyaltyService.saveLoyalty(loyalty);
    URI location = fromCurrentRequest().path("/{id}").buildAndExpand(loyaltyStored.getId()).toUri();

    return ResponseEntity.created(location).body(loyaltyStored);
  }
